...
export default function SetupProfilePage() {
  ...
  useEffect(() => {
    ...
    // Pre-fill username from Clerk
    setUsername(prev => prev || user.username || user.firstName || "");

    fetch("/api/profile/me")
      .then(r => r.json())
      .then(({ player }) => {
        if (player?.avatar_config) {
          // Already configured — skip to join
          router.replace("/join");
          return;
        }
        // Use Supabase username if set
  ...
  return (
    <div className="min-h-screen bg-mm-bg pb-10 px-5">
      ...
      <div className="pt-8 pb-4 text-center">
        <h1 className="font-marker text-2xl text-mm-cyan">Create Your Avatar</h1>
        <p className="font-boogaloo text-white/40 text-base mt-1">Your graffiti identity</p>
      </div>
