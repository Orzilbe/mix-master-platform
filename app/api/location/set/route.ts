...
async function checkAdmin(userId: string | null): Promise<{ ok: boolean; reason: string }> {
  console.log(`[admin/set] userId=${userId ?? "(not logged in)"} ADMIN_USER_IDS=${process.env.ADMIN_USER_IDS ?? "(not set)"}`);
  ...
  try {
    ...
    console.log(`[admin/set] explicit=${inExplicitList} firstPlayer=${isFirstPlayer} (first=${data?.clerk_id}) → ${ok ? "PASS" : "FAIL"}`);
    ...
    console.error("[admin/set] DB error:", err);
...
export async function POST(req: NextRequest) {
  ...
  if (!ok) {
    console.log(`[admin/set] Forbidden — ${reason}`);
  ...
  const { lat, lon, name, radius_m } = await req.json();
  const location = await upsertLocation(
    Number(lat),
    Number(lon),
    Math.min(Math.max(Number(radius_m) || 500, 50), 5000),
    name ?? "Main Venue",
  );
